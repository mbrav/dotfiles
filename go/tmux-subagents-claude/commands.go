...
func cmdPrompt(task, text string, wait, verify bool) {
	...
	if !sendPrompt(ref.PaneID, text, verify) {
		...
		exitErr(2, "prompt-not-submitted: agent '%s' pane %s. "+
...
func cmdResult(task string, wait bool) {
	...
	if wait {
		...
		stderrln("Waiting for response from '%s' (session: %s)...", ref.Name, sessionID)
	...
	if !ok {
		...
		exitErr(1, "No complete response yet for agent '%s' (session: %s)", ref.Name, sessionID)
	}
...
func waitForNewResponse(name, jsonl, baseline string) {
	...
	for {
		...
		if timedOut(deadline) {
			exitErr(2, "wait-timeout: no new response from '%s' after %.0fs", name, cfg.WaitTimeout)
		}
	}
}
...
func waitWhileBusy(name, sessionID, jsonl string) {
	...
	for {
		...
		if timedOut(deadline) {
			exitErr(2, "wait-timeout: no response from '%s' after %.0fs (session: %s)",
...
func cmdStatus(taskFilter string, all bool) {
	...
	if taskFilter != "" {
		...
		if len(filtered) == 0 {
			...
			exitErr(2, "unknown-task: %s", taskFilter)
		}
...
func printStatusTable(rows []StatusRow) {
	headers := [6]string{"PROJECT", "PANE", "TASK", "SESSION-ID", "STATUS", "CONTEXT"}
	cells := func(r StatusRow) [6]string {
		return [6]string{r.Project, r.Pane, r.Task, r.Session, r.Status, r.Context}
	}
	w := [6]int{}
	for i := range headers {
		w[i] = utf8.RuneCountInString(headers[i])
	}
	for _, r := range rows {
		c := cells(r)
		for i := 0; i < 6; i++ {
			if n := utf8.RuneCountInString(c[i]); n > w[i] {
				w[i] = n
			}
		}
	}
	printRow := func(c [6]string) {
		parts := make([]string, 6)
		for i := 0; i < 6; i++ {
			pad := w[i] - utf8.RuneCountInString(c[i])
			parts[i] = c[i] + strings.Repeat(" ", pad)
		}
...
func cmdCapture(task, mode string) {
	...
	if (mode == "" || mode == "full") && ref.Meta.SessionID != "" {
		if sessionStatuses()[ref.Meta.SessionID] == "idle" {
			stderrln("hint: '%s' is idle — `result %s` is cheaper "+
...
func killPane(pane string) bool {
	_, _, code := tmuxResult("kill-pane", "-t", pane)
	return code == 0
}
...
func cleanupAll(win string) {
	...
	st := loadWin(win)
	tasks := make([]string, 0, len(st.Agents))
	for t := range st.Agents {
...
func cmdRecap(task string) {
	...
	if !sendPrompt(ref.PaneID, "/recap", true) {
		...
		exitErr(2, "prompt-not-submitted: agent '%s' pane %s", ref.Name, ref.PaneID)
	}
