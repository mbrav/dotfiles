...
func winKey(win string) string {
	return strings.ReplaceAll(strings.ReplaceAll(win, "/", "-"), " ", "_")
}
...
func saveWin(win string, st WinState) {
	...
	if err != nil {
		...
		exitErr(1, "failed to encode state for window %q: %v", win, err)
	}
	if err := os.WriteFile(winFile(win), b, 0o644); err != nil {
		logErrorf("saveWin %s: write error: %v", win, err)
		exitErr(1, "failed to write state for window %q: %v", win, err)
	}
